...
func (jc *HttpClient) doDownloadFile(downloadFileDetails *DownloadFileDetails, logMsgPrefix string, followRedirect bool,
	...
	arch, err := archiver.ByExtension(downloadFileDetails.FileName)
	...
	if isExplode {
		err = extractFile(downloadFileDetails, arch.(archiver.Unarchiver), resp.Body, logMsgPrefix)
		return
	}
	return
}
...
func extractFile(downloadFileDetails *DownloadFileDetails, arch archiver.Unarchiver, reader io.Reader, logMsgPrefix string) error {
	...
	err = arch.Unarchive(downloadFileDetails.LocalPath, extractionPath)
	return errorutils.CheckError(err)
}
...
func (jc *HttpClient) DownloadFileConcurrently(flags ConcurrentDownloadFlags, logMsgPrefix string,
	...
	if flags.Explode {
		arch, err := archiver.ByExtension(flags.FileName)
		if err != nil {
			return nil, err
		}
